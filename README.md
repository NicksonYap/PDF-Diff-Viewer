...
This is done on purpose: imagine a word that, between the two versions, changes its location because it goes to the next line, and here you want to see it as unchanged. Therefore, it's a feature, not a bug. Changes in the font (color, formatting, etc.) is ignored in the same way. In other words, changes in everything else that is not text (say, images, shapes, etc.) are ignored.
